    ...
    var HZRecorder = function (stream, config) {
        ...
        var audioData = {
            ...
            , compress: function () { //合并压缩
                ...
                //压缩
				console.log("Input samp rate: " + this.inputSampleRate);
                console.log("Output samp rate: " + this.outputSampleRate);
                ...
                var index = 0, j = 0.0, int_j = 0;
                while (index < length) {
                    result[index] = data[int_j];
                    j += compression;
					int_j = Math.round(j)
                    index++;
                }
                    ...
                    , function (error) {
                        switch (error.code || error.name) {
                            ...
                            case 'NotSupportedError':
                                HZRecorder.throwError('瀏覽器不支援硬體設備。');
                                break;
							...
							case 'InvalidStateError':
                                console.log('INVALID_STATE_ERROR無法使用麥克風，請藉由重新整理頁面來解決此問題。');
								//location.reload();
                                break;
                            default:
                                HZRecorder.throwError('無法打開麥克風，錯誤代碼為:' + (error.code || error.name));
                                break;
                        }
                    });
            } else {
